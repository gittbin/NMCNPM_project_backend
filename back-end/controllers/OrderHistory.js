...
const template=(listOrder)=>{
  ...
  return `dear ${listOrder[0].supplier}\n`+needsEmail.map((order)=>`tôi muốn nhập ${order.quantity} sản phẩm  ${order.name}\n`)+'Cảm ơn vì đã xem'
}
const saveOrderHistory = async (req, res) => {
  ...
  try {
    ...
    for (const suppOrders of listOrder) {
      ...
      await sendEmail('nguyenkhoe2k4@gmail.com','nhập hàng',template(suppOrders));
